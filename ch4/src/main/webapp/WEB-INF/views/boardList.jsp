...
<head>
    <meta charset="UTF-8">
    <title>chae boardList.jsp</title>
    <!--원래 /css/menu.css였는데 에러떠서 /resources 붙이니까 빨간줄안뜸-->
...
<script>
    //BoardController의 remove()에서 모델에 담아 넘겨준 msg 받기
    let msg = "${msg}"
    if(msg=="list error")  alert("게시물 목록을 가져오는데 실패했습니다. 다시 시도해 주세요.");
    if(msg=="read error")  alert("삭제되었거나 없는 게시물입니다.");
    if (msg == "delete ok") alert("성공적으로 삭제되었습니다.")
    if (msg == "delete error") alert("삭제에 실패했습니다.")
    if (msg == "write ok") alert("성공적으로 등록되었습니다.")
    if (msg == "write error") alert("등록에 실패했습니다.")
    if(msg=="modify completed")    alert("성공적으로 수정되었습니다.");
</script>
...
<%--            <tr>--%>
<%--                    &lt;%&ndash;            EL을 사용하여 ${boardDto.bno}와 같이 속성 값을 가져올 때는 boardDto에 실제로 해당 속성을 반환하는 getBno() 메소드가 정의되어 있어야함.--%>
<%--                                    EL은 JavaBeans 규약을 따르는 클래스에서 getter 메소드를 사용하여 속성 값을 가져오는 것을 기반으로 동작함.&ndash;%&gt;--%>
<%--                        <td class="no">${boardDto.bno}</td>--%>
<%--                <!--page값과 pageSize값은 BoardController의 list()가 줌-->--%>
<%--                <td>--%>
